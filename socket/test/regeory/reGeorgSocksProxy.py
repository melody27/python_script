...
class session(Thread):
    ...
    def parseSocks4(self, sock):
        ...
        if cmd == "\x01":  # Connect
            ...
            targetPort = ord(targetPort[0]) * 256 + ord(targetPort[1])
            target = sock.recv(4)
            sock.recv(1)                                        # limit the after the hostname a bytes 
            target = ".".join([str(ord(i)) for i in target])
            if target == '0.0.0.1':
                print('target is here:',target)
                target = sock.recv(1024)[:-1]           # here the true target        
                print("here is the last recv",target)
            serverIp = target
            print("here is a target",target)          # if this is the socks4a,so there target is 0.0.0.1
            
            try:
                serverIp = gethostbyname(target)
            except:
                log.error("oeps")
            serverIp = "".join([chr(int(i)) for i in serverIp.split(".")])  # the severIp is the servre'ip but target is the hostname
            print('here the serverIP :',serverIp)
            self.cookie = self.setupRemoteSession(target, targetPort)           # here the target is hostname
            if self.cookie:
                sock.sendall(chr(0) + chr(90) +chr(targetPort / 256) + chr(targetPort % 256) + serverIp)
                return True
            else:
